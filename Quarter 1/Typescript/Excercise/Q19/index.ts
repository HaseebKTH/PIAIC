let invitees : string[] = ["Marie Curie", "Nikola Tesla", "Ibn Battuta", "Reinhold Messner", "Antoni Gaudí"];

for(let i=0; i< invitees.length; i++)
{
    console.log(`Dear Mr/Mrs ${invitees[i]} \nI'm sending you my warmest invitation to this dinner party. Your presence is all that is required to make this party a success.\nRegards,\nHaseeb Aslam Butt\n\n`)
}

console.log(`Total ${invitees.length} guests are invited for our dinner.`)