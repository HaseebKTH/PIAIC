...
async function main() {
    ...
    if (choice == 'Absolute Time') {
        ...
        let deltaTime: Date = await getTargetTime(choice);
        targetTime = new Date();
...
async function getTargetTime(choice: string) {
...
function calculateDiffSec(date1: Date, date2: Date): number {
